// https://github.com/wildan444/whatsapp-bot
...
Bot ini open source loh! kakak bisa cek di https://github.com/wildan444/whatsapp-bot (jika ingin mengedit mohon untuk tidak hilangankan link ini)
...
- WA: *wa.me/81386615459*`;
		...
		case "!stiker":
		{
			...
			const sticker = new WSF.Sticker("./" + imagePath, { crop: false, pack: "github.com/wildan444", author: conn.user.name });
			await sticker.build();
		...
		case "!textstiker":
		{
			...
			const sticker = new WSF.Sticker(response.data.image, { crop: false, pack: "github.com/wildan444", author: conn.user.name });
			await sticker.build();
			const bufferImage = await sticker.get();
			conn.sendMessage(senderNumber, bufferImage, MessageType.sticker, { quoted: message });
			break;
